...
InputHeatmap <- setClass(
	...
	prototype = list(
		...
		group_top_annotation = list(),
		group_left_annotation = list(),
		layer_symbol = tibble(column = integer(), row = integer(), shape = integer(), size = numeric(), color = character()),
		layer_text = tibble(column = integer(), row = integer(), text = character(), size = numeric(), color = character())
	)
) 